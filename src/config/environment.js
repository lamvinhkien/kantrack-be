import 'dotenv/config'

export const env = {
  MONGODB_URI: process.env.MONGODB_URI,
  DATABASE_NAME: process.env.DATABASE_NAME,

  LOCAL_DEV_APP_HOST: process.env.LOCAL_DEV_APP_HOST,
  LOCAL_DEV_APP_PORT: process.env.LOCAL_DEV_APP_PORT,

  WEBSITE_DOMAIN_DEV: process.env.WEBSITE_DOMAIN_DEV,
  WEBSITE_DOMAIN_PROD: process.env.WEBSITE_DOMAIN_PROD,

  MAILER_SEND_SUPPORT_EMAIL: process.env.MAILER_SEND_SUPPORT_EMAIL,
  MAILER_SEND_API_KEY: process.env.MAILER_SEND_API_KEY,
  ADMIN_SENDER_EMAIL: process.env.ADMIN_SENDER_EMAIL,
  ADMIN_SENDER_NAME: process.env.ADMIN_SENDER_NAME,

  ACCESS_TOKEN_SECRET_SIGNATURE: process.env.ACCESS_TOKEN_SECRET_SIGNATURE,
  ACCESS_TOKEN_LIFE: process.env.ACCESS_TOKEN_LIFE,
  REFRESH_TOKEN_SECRET_SIGNATURE: process.env.REFRESH_TOKEN_SECRET_SIGNATURE,
  REFRESH_TOKEN_LIFE: process.env.REFRESH_TOKEN_LIFE,

  OTP_EXPIRE_MINUTES: process.env.OTP_EXPIRE_MINUTES,
  OTP_RESEND_EXPIRES: process.env.OTP_RESEND_EXPIRES,

  CRON_REMINDER_TIME_PROD: process.env.CRON_REMINDER_TIME_PROD,
  CRON_REMINDER_TIME_DEV: process.env.CRON_REMINDER_TIME_DEV,

  CLOUDINARY_CLOUD_NAME: process.env.CLOUDINARY_CLOUD_NAME,
  CLOUDINARY_API_KEY: process.env.CLOUDINARY_API_KEY,
  CLOUDINARY_API_SECRET: process.env.CLOUDINARY_API_SECRET,

  BUILD_MODE: process.env.BUILD_MODE
}
